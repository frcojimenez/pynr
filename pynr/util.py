...
def FFT_ZeroPadded_Windowed(data,nleft=10,nright=10,x_low=-100,sigma_low=50,x_high=100,sigma_high=50):
    windowed = Window_Tanh(data,x_low,sigma_low,x_high,sigma_high)
    zero_padded = ZeroPadTimeSeries(windowed,nleft,nright)

    print("Start of zero-padded time series before FT = ", zero_padded[0,0]);
    print("length of original time series before FT = ", len(windowed));
    print("length of zero-padded time series before FT = ", len(zero_padded));
...
def FFT_ZeroPadded_Windowed_v2(wave,omega0):

    initialPeriod=2*np.pi/omega0
    print("initialPeriod = ",initialPeriod)
    print("low freq cutoff = 1/initialPeriod = ",1./initialPeriod)
    f0=1./initialPeriod
    fcut=0.2
    tStartFourierInput=-16727216
    
    dt=wave[1,0]-wave[0,0]
    initialTime = wave[0,0]
    finalTime = wave[-1,0]
    newStartTime = -16727216
    newEndTime = 500  
    
    nleft  = np.int((initialTime - newStartTime)/dt)
    nright = np.int((newEndTime - finalTime)/dt)
    extra = np.int(2**(np.floor(np.log2(len(wave)+nleft+nright))+1)-(len(wave)+nleft+nright))
    
    print('nleft: ',nleft+extra)
    print('nright: ',nright)

    x_low = initialTime + 20*np.pi/(omega0)
    x_high = wave[-1,0]-10
    sigma_low = 4*np.pi/omega0
    sigma_high = 20
    
    print('Starting windowing at: ', x_low,'  with width: ',sigma_low)
    print('Starting windowing at: ', x_high,'  with width: ',sigma_high)
    
    ft=FFT_ZeroPadded_Windowed(wave,nleft+extra,nright,x_low = x_low,sigma_low=sigma_low,x_high = x_high,sigma_high=sigma_high)
    freqs= ft[:,0]
    ft = ft[freqs>0]
